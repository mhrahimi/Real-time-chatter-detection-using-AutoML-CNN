...
if numel(data)-dSize < dSize
    thisData = [];
    while numel(thisData) < dSize
        thisData = [thisData, data];
    end
    thisData = thisData(1:dSize);
    thisImOut = param.tfFunction(thisData, properties, param);
    eachLabel = classify(param.net, thisImOut*255);
    out = eachLabel;
else
    out(i*dSize:numel(data)) = eachLabel(i);
end

function [lgraph, options] = HosseinNet2in(valImgs, inParam, optParam)
%% Deafualt values
...
%% Blocks
% Convolution Blocks
convBlock{1} = [imageInputLayer(inputSize, "Name","imageinput", ...
    'Normalization', 'none')
...
%% CNN layers assemble
lgraph = layerGraph();
lgraph = addLayers(lgraph, imageInputLayer([1 1 1], "Name","TPF", ...
    'Normalization', 'none'));
...
beforeConcatModule = [convModule; NNBlock{1}];
...
lgraph = connectLayers(lgraph,"TPF","concat/in2");
